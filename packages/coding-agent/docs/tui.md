...
The first frame is already the two-divider terminal. `control.start()` only stages the exact read-only command in tmux and leaves it waiting at the terminal cursor; it does not execute sudo. `app.privilege.confirm` calls `control.execute()` to release that staged command, while `app.privilege.cancel` closes the temporary view without execution. Running/authenticating states forward direct terminal bytes only through `PrivilegeTerminalControl.sendSensitive(Buffer)` and never store them in component state, render output, cache, Session, or response objects.

After authentication, the temporary terminal detaches and restores the editor while `PrivilegeRuntime` continues waiting for command completion, emitting non-sensitive output through ordinary partial Tool updates and the work log. If sudo uses cached credentials and shows no prompt, the component detaches after a bounded stable-running interval. Interactive root shells such as `sudo bash`, `sudo sh`, `sudo -i`, and `sudo -s` are blocked so detach cannot leave a hidden elevated shell. Styled tmux capture preserves ANSI colors in the temporary view without changing the plain marker and authentication parsers. Local privilege sessions use an isolated tmux server so the configured user shell, cwd, environment, and normal startup files match a fresh user tmux session without `env -i`, `--noprofile`, or `--norc`.

This component is a trusted built-in boundary, not an extension password prompt. Extensions and SDK hosts must not collect sudo passwords in forms, return values, Tool parameters, or RPC messages. A host integration stages with `control.start()`, waits for the user's Enter or Escape decision, calls `control.execute()` only for Enter, forwards authentication bytes through `control.sendSensitive(Buffer)`, and may return `completed` after authentication so the Runtime continues waiting in the background. Short successful output is returned directly; failed, diagnostic, or over-100-line output uses the shared `review.model` pipeline while the complete output remains in the work log.
