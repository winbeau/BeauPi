...
BeauPi also uses versioned internal custom facts. `beaupi.privilege.fact` stores only completed controlled-sudo result metadata such as request/tool ids, command, route, target reference, timestamps, exit status, Monitor/log paths, truncation, stable diagnostics, and optional terminal review metadata. It never stores authentication input, a pending request, a sudo ticket, or a reusable grant. Branch navigation and resume project these historical facts into Task Ledger but do not restore an interaction.
