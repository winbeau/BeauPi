...
export type AgentSessionEvent =
	...
	| { type: "compaction_progress"; deltaCharacters: number }
		...
		try {
			...
			if (extensionCompaction) {
				...
				const result = await compact(
					...
					(deltaCharacters) => this._emit({ type: "compaction_progress", deltaCharacters }),
				);
		...
		try {
			if (hasPotentialShellPrivilege(resolvedCommand)) {
				...
				const result: BashResult = {
					...
					status: privilege.status === "cancelled" ? "cancelled" : privilege.ok ? "completed" : "failed",
