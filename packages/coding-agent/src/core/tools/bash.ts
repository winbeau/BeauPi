...
import { hasPotentialShellPrivilege, inspectShellPrivilege } from "../privilege/shell-inspection.ts";
...
export interface BashToolDetails {
	...
	/** Neutral failure category for non-zero exits; absent for completed runs. */
...
export class BashToolExecutionError extends Error {
	...
	constructor(message: string, failureCategory: ExecutionFailureCategory, exitCode?: number | null) {
		super(message);
		...
		async execute(
			...
			try {
				...
				if (exitCode !== 0 && exitCode !== null) {
					const failureCategory = bashFailureCategory(exitCode, outputText) ?? "command_exit";
					throw new BashToolExecutionError(
				...
				return {
					...
					details: {
						command,
						exitCode,
						status: bashExecutionStatus({ exitCode, cancelled: false, timedOut: false }),
						...(details ?? {}),
					},
				};
			} finally {
