...
describe("detectInstallMethod", () => {
	test("detects pnpm from Windows .pnpm install paths", () => {
		...
		expect(getUpdateInstruction("@earendil-works/pi-coding-agent")).toBe(
			"Run: pnpm install -g --config.minimumReleaseAge=0 @earendil-works/pi-coding-agent",
		);
	});
	...
	test("self-updates npm installs from custom prefixes", () => {
		...
		expect(command).toEqual({
			command: "npm",
			args: ["--prefix", prefix, "install", "-g", "--min-release-age=0", "@earendil-works/pi-coding-agent"],
			display: `npm --prefix ${prefix} install -g --min-release-age=0 @earendil-works/pi-coding-agent`,
		});
	});
	...
	test("self-updates exact npm versions without uninstalling the current package", () => {
		...
		expect(command).toEqual({
			command: "npm",
			args: ["--prefix", prefix, "install", "-g", "--min-release-age=0", "@earendil-works/pi-coding-agent@1.2.3"],
			display: `npm --prefix ${prefix} install -g --min-release-age=0 @earendil-works/pi-coding-agent@1.2.3`,
		});
	});
	...
	test("self-updates renamed packages from the current install prefix", () => {
		...
		expect(command).toEqual({
			command: "npm",
			args: ["--prefix", prefix, "install", "-g", "--min-release-age=0", "@new-scope/pi"],
			display: `npm --prefix ${prefix} uninstall -g @mariozechner/pi-coding-agent && npm --prefix ${prefix} install -g --min-release-age=0 @new-scope/pi`,
			steps: [
				{
					command: "npm",
				...
				},
				{
					command: "npm",
					args: ["--prefix", prefix, "install", "-g", "--min-release-age=0", "@new-scope/pi"],
					display: `npm --prefix ${prefix} install -g --min-release-age=0 @new-scope/pi`,
				},
			],
		});
	});
	...
	test("quotes npm self-update display paths", () => {
		...
		expect(command?.display).toBe(
			`npm --prefix "${prefix}" install -g --min-release-age=0 @earendil-works/pi-coding-agent`,
		);
	});
	...
	test("does not infer Windows npm custom prefixes from package paths", () => {
		...
		expect(detectInstallMethod()).toBe("npm");
		expect(getUpdateInstruction("@earendil-works/pi-coding-agent")).toBe(
			"Run: npm install -g --min-release-age=0 @earendil-works/pi-coding-agent",
		);
	});
	...
	test("self-updates bun global installs from bun pm bin", () => {
		...
		expect(command).toEqual({
			command: "bun",
			args: ["install", "-g", "--minimum-release-age=0", "@earendil-works/pi-coding-agent"],
			display: "bun install -g --minimum-release-age=0 @earendil-works/pi-coding-agent",
		});
	});
	...
	test("self-updates renamed pnpm global installs by removing the old package first", () => {
		...
		expect(command).toEqual({
			command: "pnpm",
			args: ["install", "-g", "--config.minimumReleaseAge=0", "@new-scope/pi"],
			display:
				"pnpm remove -g @mariozechner/pi-coding-agent && pnpm install -g --config.minimumReleaseAge=0 @new-scope/pi",
			steps: [
				{
					command: "pnpm",
				...
				},
				{
					command: "pnpm",
					args: ["install", "-g", "--config.minimumReleaseAge=0", "@new-scope/pi"],
					display: "pnpm install -g --config.minimumReleaseAge=0 @new-scope/pi",
				},
			],
		});
	});
	...
	test("self-updates renamed bun global installs by removing the old package first", () => {
		...
		expect(command).toEqual({
			command: "bun",
			args: ["install", "-g", "--minimum-release-age=0", "@new-scope/pi"],
			display:
				"bun uninstall -g @mariozechner/pi-coding-agent && bun install -g --minimum-release-age=0 @new-scope/pi",
			steps: [
				{
					command: "bun",
				...
				},
				{
					command: "bun",
					args: ["install", "-g", "--minimum-release-age=0", "@new-scope/pi"],
					display: "bun install -g --minimum-release-age=0 @new-scope/pi",
				},
			],
		});
	});
