...
## M10：Policy Engine（已移除）

状态：已实现，随后在 Trusted-Local Runtime 升级中删除（2026）。

M10 的 Core Policy Engine（PolicyRuntime、失败预算、敏感路径分类、Footer advisory、`beaupi.policy.fact`）不再存在：工具调用不经过任何授权门。仍保留的中性执行事实迁移到 `core/execution/`（失败分类 `ExecutionFailureCategory`、Dynamic Task 的 workspace mutation 分类）。
